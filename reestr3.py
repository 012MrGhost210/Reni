...
def format_excel_file(worksheet, total_rows, enable_filter, keywords_count, search_mode):
    """
...
def create_excel_report(files_data, output_path, total_processed, matching_files, 
                       enable_filter, keywords_list, search_mode):
    """
    ...
    try:
        ...
        format_excel_file(worksheet, total_rows, enable_filter, len(keywords_list), search_mode)
        ...
        if enable_filter:
            ...
            worksheet.cell(row=info_row, column=1, 
                          value=f"Режим поиска: {mode_text}")
            worksheet.cell(row=info_row, column=1).font = Font(bold=False, color="4472C4", size=11)
...
def analyze_directory_files():
    ...
    if ENABLE_KEYWORD_FILTER:
        # Информация о режиме поиска
        mode_text = "ТОЧНОЕ СОВПАДЕНИЕ СЛОВ (не содержит внутри других слов)" if SEARCH_MODE == "exact" else "ПОИСК ПОДСТРОКИ (может быть частью слова)"
    ...
    if files_data:
        ...
        success = create_excel_report(files_data, excel_path, total_processed, 
                                     matching_files, ENABLE_KEYWORD_FILTER, 
                                     keywords_list, SEARCH_MODE)
        ...
        if ENABLE_KEYWORD_FILTER:
            mode_text = "точное совпадение" if SEARCH_MODE == "exact" else "поиск подстроки"
            print(f"   🔍 Режим поиска: {mode_text}")
