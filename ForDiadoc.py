...
SOURCES_CONFIG = [
    {
        # Источник 1: УК Первая (обычные файлы)
        ...
        "manager": "УК Первая",
        "type": "files",  # только обычные файлы
    },
    {
        # Источник 2: УК Райффайзен (архивы + обычные файлы)
        ...
        "manager": "Райффайзен Капитал",
        "type": "mixed",  # и архивы, и обычные файлы
    },
]

# Маппинг названий управляющих (исходное -> красивое) - для автоматического определения
MANAGER_MAPPING = {
...
ALLOWED_EXTENSIONS = {'.pdf', '.xlsx', '.xls', '.doc', '.docx', '.txt', '.xml', '.csv'}
...
# ==================== КОНФИГУРАЦИЯ ФАЙЛОВ (для обычных файлов) ====================
...
FILE_RULES = [
    ...
    },
    {
        ...
        "date_regex": r"(\d{4}\.\d{2}\.\d{2})_27011_журнал учета ДС",
        "date_format": "%Y.%m.%d",
        "destination_template": r"6.Журнал учета операций\*ГГГГ*\*Месяц*\*Управляющий*",
        "identifier": "журнал учета ДС",
    },
    {
        # Правило 4: Журнал учета ДС (новый тип - без фигурных скобок)
        "name_pattern": "",
        "date_regex": r"(\d+)_Журнал учета дс_(\d{8})",
        "date_format": "%d%m%Y",
        "destination_template": r"6.Журнал учета операций\*ГГГГ*\*Месяц*\*Управляющий*\*Портфель*",
        "identifier": "Журнал учета дс",
        "has_portfolio": True,
    },
]
...
def extract_date_from_filename(filename, rule):
    ...
    # Для правила 4 (Журнал учета дс) - специальная обработка
    if rule.get("has_portfolio", False):
        match = re.search(rule["date_regex"], filename)
        if match:
            portfolio_code = match.group(1)
            date_str = match.group(2)
            try:
                date_obj = datetime.strptime(date_str, rule["date_format"])
                return date_obj, portfolio_code, True
            except ValueError:
                return None, None, False
        return None, None, False
    
    # Для остальных правил
    if rule["name_pattern"] and not filename.startswith(rule["name_pattern"]):
        return None, None, False
    
    name_part = filename[len(rule["name_pattern"]):] if rule["name_pattern"] else filename
...
def build_destination_path(date_obj, rule, manager_name, portfolio_code, original_filename):
    """Строит путь назначения для обычного файла"""
    ...
    # Если есть портфель, подставляем его
    if portfolio_code:
        dest_path = dest_path.replace("*Портфель*", portfolio_code)
...
def process_regular_files(source_dir, manager_name, is_raiffaisen=False):
    """Обрабатывает обычные файлы"""
    print(f"\n📁 Обработка обычных файлов")
    ...
    for filename in files:
        ...
        # Пропускаем архивы (их обрабатывает отдельная функция)
        if filename.lower().endswith(('.zip', '.rar')):
            print(f"   ⏭️ Пропускаем архив (будет обработан отдельно): {filename}")
            continue
        ...
        for rule in FILE_RULES:
            date_obj, portfolio_code, found = extract_date_from_filename(filename, rule)
            if found:
                found_rule = rule
                break
        ...
        dest_path = build_destination_path(date_obj, found_rule, manager_name, portfolio_code, filename)
...
def open_zip(zip_path):
    """Открывает zip-архив с правильной кодировкой"""
...
def extract_archive_file_info(filename):
    """Извлекает информацию из имени файла внутри архива"""
    for rule in ARCHIVE_FILE_RULES:
...
def build_archive_destination_path(date_obj, rule, manager_name, portfolio_code, broker_code, filename):
    """Строит путь назначения для файла из архива"""
    year = str(date_obj.year)
    month_folder = format_month_folder(date_obj)
...
def process_archives(source_dir, manager_name):
    ...
    for archive_name in archives:
        ...
        try:
            zip_ref, need_decode = open_zip(archive_path)
            with zip_ref:
                ...
                for info in infos:
                    ...
                    file_date, file_portfolio, broker_code, rule = extract_archive_file_info(base_name)
                    ...
                    dest_path = build_archive_destination_path(
            ...
            print(f"      ❌ Файл не является zip-архивом: {archive_path}")
            skipped += 1
        except Exception as e:
...
def process_all_sources():
    ...
    print("=" * 70)
    print("📁 Начинаем обработку всех источников")
    print(f"📂 Базовый путь назначения: {BASE_DEST_DIR}")
    print("=" * 70)
    ...
    for source_config in SOURCES_CONFIG:
        ...
        manager_name = source_config.get("manager") or get_manager_name(source_dir)
        source_type = source_config.get("type", "files")
        
        print(f"\n{'='*70}")
        print(f"📍 Обработка источника: {source_dir}")
        print(f"👤 Управляющий: {manager_name}")
        ...
        if source_type == "files":
            ...
            processed, skipped = process_regular_files(source_dir, manager_name)
        elif source_type == "archives":
            # Только архивы
            processed, skipped = process_archives(source_dir, manager_name)
        elif source_type == "mixed":
            # И обычные файлы, и архивы
            print(f"\n📄 Обработка обычных файлов...")
            processed1, skipped1 = process_regular_files(source_dir, manager_name)
            print(f"\n📦 Обработка архивов...")
            processed2, skipped2 = process_archives(source_dir, manager_name)
...
if __name__ == "__main__":
    process_all_sources()
256_Журнал учета дс_09062026 - пример файла
*код портфеля*_Журнал учета дс_*ДДММГГГГ* - пример логики в файле
M:\Финансовый департамент\Treasury\Отчеты Брокера и Спецдепозитария\6.Журнал учета операций\*ГГГГ*\*Месяц*\*Управляющий*\*Портфель* - логика пути
M:\Финансовый департамент\Treasury\Отчеты Брокера и Спецдепозитария\6.Журнал учета операций\2026\06.Июнь 2026\Райффайзен Капитал\256-03.1-ДУ24 - пример пути
Но тут не в архиве, тут просто файл в нужном корне
import os
import shutil
import re
import zipfile
